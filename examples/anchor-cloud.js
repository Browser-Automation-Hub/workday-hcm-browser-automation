/**
 * anchor-cloud.js — AnchorBrowser cloud example for Workday HCM
 *
...
const { login_workday, run_report } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting Workday HCM automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_workday(page, {});
      log('Action 1 result:', data1);

      const data2 = await run_report(page, {});
