# Contributing to Workday HCM Browser Automation
...
- **Broken selectors**: Workday HCM updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real Workday HCM instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.WORKDAY_URL}/path/to/section`);
...
When you find working selectors for Workday HCM, document them:

```javascript
// Workday HCM selectors (verified YYYY-MM-DD)
...
Workday HCM updates its UI periodically. When selectors break:
